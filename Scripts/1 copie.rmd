...
u %>% 
  ...
  theme_bw() +
  labs(x = "State ID", y = "Sales", size = 10) +
...
w %>%
  ...
  theme_bw() +
  labs(x = "category ID", y = "Sales", size = 10) +
...
q %>%
  ...
  theme_bw() +
  labs(x = "departement ID", y = "Sales", size = 10) +
...
r <- tdf3%>%
  group_by(month)%>%
  mutate(avg_sale_month = median(Sales_per_day, na.rm = T))
  
ggplot(r)+
  geom_col(aes(x = as.factor(month), y = avg_sale_month), fill = "steelblue3") +
  theme_bw() +
  labs(x = "Month", y = "Median sales", size = 10) +
  labs(title = "Median sales per month") +
  theme(plot.title = element_text(face = "bold", hjust = 0.5, size = 10)) +
  theme(legend.position = "None")+
...
It seems that sales are slightly higher between March and June.
...
all years appear similar in terms of average sales, although there are variations between years. There are some common points to all years, such as lower median sales in January, a peak around August and a drop between October and November. 
Note that not all products were on sale from January 2011, which is probably why 2011 is lower than other years. 
...
prices_TX1 %>%
  ...
  geom_line(aes(wm_yr_wk, avg_price, color = category)) +
    theme_bw() +
  labs(x = "Years", y = "average prices", size = 10)+
...
events %>%
  ...
  labs(x = "Event type", y = "Number of events", title = "Proportion of events by type ?") +
       theme(plot.title = element_text(hjust = 0.5, face = "bold"))
